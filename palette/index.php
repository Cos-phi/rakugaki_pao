...
<body>
    <div id="app" style="max-width: 480px; margin: 32px auto;">
        ...
        <div class="output-row">
            ...
            <button id="copyPaletteUrl">パレットをコピー</button>
        </div>
        <div class="output-row" id="paletteUrlPreview" style="margin-top:8px;"></div>
    ...
    <script>
        ...
        const copyPaletteUrlBtn = document.getElementById('copyPaletteUrl');
        ...
        function getPaletteUrl() {
            const url = `${location.origin}${location.pathname}?colors=${encodePalette(paletteStr)}`;
            return url;
        }
        // パレットURLコピー
        ...
        // パレットURLプレビュー表示＋OGP用画像
        function renderPaletteUrlPreview() {
            const url = getPaletteUrl();
            const svg = generatePaletteSVG();
            const svgBase64 = btoa(unescape(encodeURIComponent(svg)));
            const svgDataUri = `data:image/svg+xml;base64,${svgBase64}`;
            paletteUrlPreview.innerHTML = `<input type="text" value="${url}" style="width:90%" readonly>\n` +
                `<img src='${svgDataUri}' width='420' height='42' style='vertical-align:middle;margin-left:8px;'>`;
        }
        // SVG生成（10色横並び）
        function generatePaletteSVG() {
            let colors = [blackPenColor, backgroundColor, ...paletteColors, changeableColor].slice(0,10);
            let rects = colors.map((c,i)=>`<rect x='${i*42}' y='0' width='42' height='42' fill='${c}' stroke='#36364A'/>`).join('');
            return `<svg xmlns='http://www.w3.org/2000/svg' width='420' height='42'>${rects}</svg>`;
        }
